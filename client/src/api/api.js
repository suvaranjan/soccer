...
// export const baseUrl = "http://149.28.150.230:3000/api";
...
export async function getUserMatches(token, currentPage, limit) {

    const response = await axios.get(`${baseUrl}/match/user-matches?page=${currentPage}&limit=${limit}`, {
        headers: getHeader(token),
    });
...
export async function setAsDefaultTeam(token, teamId) {

    const response = await axios.post(`${baseUrl}/manager/make-default-team`, { teamId }, {
        headers: getHeader(token),
    });
