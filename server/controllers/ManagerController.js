...
const addPlayer = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ msg: 'Failed to create player', error: error.message });
    }
};
...
const getManagerTeams = async (req, res) => {
    try {
        ...
        // Extract the teams
        const { teams, defaultTeam } = manager;

        // Send response with the populated teams
        res.status(200).json({ teams, defaultTeam });
    } catch (error) {
        console.error(error);
...
const setDefaultTeam = async (req, res) => {
    try {
        ...
        // Check if the team exists in manager's teams
        if (!manager.teams.includes(teamId)) {
            return res.status(404).json({ msg: 'Team not found in manager\'s teams' });
        }

        // Set the team as default
        manager.defaultTeam = teamId;
        await manager.save();

        res.status(200).json({ msg: 'Default team updated successfully', manager });
    } catch (error) {
        console.error('Error setting default team:', error.message);
        res.status(500).json({ msg: 'Internal Server Error' });
    }
};
...
module.exports = {
    managerBasicInfoUpdate, getManagerProfile, addPlayer, getMyPlayers, getManagerTeams, managerPlayerUpdate, sendContractOfferToPlayer, getManagerTeamsForReminder, getManagerReminders, markReminderAsFinishedForUser, setDefaultTeam
};
