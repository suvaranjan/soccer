...
const isPlayerInATeam = async (req, res) => {
    try {
        ...
        if (team) {
            return res.status(200).json({ msg: "Player is in a team", team: team.name, playerInATeam: true });
        } else {
            return res.status(200).json({ msg: "Player is not in any team", playerInATeam: false });
        }
    } catch (error) {
        console.log("Error checking player's team status:", error.message);
        res.status(500).json({ msg: "Internal Server Error" });
    }
};
...
module.exports = {
    getPlayerProfile, playerBasicInfoUpdate, playerStrengthUpdate, isPlayerInATeam, findPlayersByKeyword, fetchTeamInvitations, rejectTeamInvitation, acceptTeamInvitation, getAllPlayers, getPlayersBySearch, getPlayerById, checkPlayerBelongsToManager, SendJoinReqToTeam
};
