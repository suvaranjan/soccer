...
const createMatch = async (req, res) => {
    try {
        ...
        const { referees, date, location, type, field, fees, description, photos, playerNeed } = req.body;

        // Validate required fields
        if (!referees || !date || !location || !type || !field || !playerNeed || !description || !fees || !photos) {
        ...
        // Fetch referees and create notifications
        const refereePromises = referees.map(async refereeId => {
            // Find referee and populate user details
            const referee = await Referee.findById(refereeId).populate('user');
            ...
            const message = 'You are invited to a match';
            const redirectUrl = `/matches/${newMatch._id}`;
            await createNotification(referee.user._id, message, redirectUrl, 'social');
        });
...
const getMatchDetails = async (req, res) => {
    try {
        ...
        const match = await Match.findById(matchId)
            .select('date location type fees status pendingTeams maxTeams confirmedTeams pendingPlayers confirmedPlayers description photos referees field playerNeed createdBy')
            .populate({
                path: 'pendingTeams',
...
const getUserMatches = async (req, res) => {
    try {
        ...
        if (user.role === 'player') {
            ...
            totalCount = await Match.countDocuments({
                $or: [
                    { createdBy: userId },
                    { 'confirmedPlayers.player': player._id }
                ]
            });
            ...
            const teamManager = await TeamManager.findOne({ user: userId }).lean();
            if (!teamManager) {
            ...
            totalCount = await Match.countDocuments({
                $or: [
                    { createdBy: userId },
                    { 'confirmedTeams.team': { $in: teamManager.teams } }
                ]
            });
            ...
            const referee = await Referee.findOne({ user: userId }).lean();
            if (!referee) {
